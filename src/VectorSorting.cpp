...
void selectionSort(vector<Bid>& bids) {
	...
	for (unsigned pos = 0; pos < bids.size(); ++pos){
		min = pos; // set min to current position

		// loop ahead to find min index
		for (unsigned j = pos + 1; j < bids.size(); ++j) {
			//if value at index is lower than min
			if(bids.at(j).title.compare(bids.at(min).title) < 0) {
				// set min index
				min = j;
			}
		}
		// if the current index is not the same min index, swap values
		if (min != pos) {
